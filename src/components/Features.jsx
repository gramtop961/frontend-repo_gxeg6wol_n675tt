  ...
  return (
    <section id="solutions" className="relative py-20">
      ...
      <div className="relative max-w-7xl mx-auto px-6">
        ...
        <p className="mt-3 text-neutral-300 text-center max-w-2xl mx-auto">
          From endpoints to cloud, we harden your attack surface and respond in minutes, not hours.
        </p>

        <div className="mt-12 grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map(({ icon: Icon, title, desc }) => (
            <div key={title} className="group rounded-2xl border border-white/10 bg-black/60 p-6 hover:border-white/30 transition-all">
              <div className="w-11 h-11 rounded-xl bg-white/5 border border-white/20 flex items-center justify-center mb-4 shadow-[0_0_20px_rgba(255,255,255,0.07)]">
                <Icon className="w-5 h-5 text-neutral-200" />
              </div>
              <h3 className="text-white font-semibold mb-1">{title}</h3>
              <p className="text-neutral-300/90 text-sm">{desc}</p>
            </div>
          ))}
        </div>
      </div>
